...
export default async function QuizPage({
    ...
    searchParams: { topic?: string; fromPdf?: string };
}) {
    const params = await searchParams;
    const topic = params.topic;
    const fromPdfParam = params.fromPdf;

    // Define se o quiz veio de um PDF (com base no parâmetro da URL)
    const isFromPdf = fromPdfParam === 'true';

    // Se não houver tópico, redireciona para a página inicial
    if (!topic) {
        redirect('/');
    }
    ...
    return (
        <main className="relative flex min-h-screen flex-col items-center justify-center p-4 md:p-6">
            ...
            <Suspense fallback={<div>Carregando quiz...</div>}>
                <InteractiveQuiz quizData={quizData} topic={topic} isFromPdf={isFromPdf} />
            </Suspense>
        </main>
    );
}
